...
echo:
echo:
echo Check troubleshoot area to readme file from main menu.
echo:
echo:
ping 127.0.0.1 -n 10
)
...
cls
color 07
title  Internet Download Manager (IDM) 6.xx Activator or Resetter v%iasver%
...
echo:            ___________________________________________________
echo:         
call :_color2 %_White% "             " %_Green% "按键盘数字选择 [1,2,3,4,5,6,7]"
choice /C:1234567 /N
set _erl=%errorlevel%

if %_erl%==7 exit /b
if %_erl%==6 call :readme&goto MainMenu
...
%psc% "$f=[io.file]::ReadAllText('!_batp!') -split \":%~1\:.*`r`n\"; [io.file]::WriteAllText('%~2',$f[1].Trim(),[System.Text.Encoding]::ASCII);"
exit/b
...
_________________________________
 ...
 - Now install the IDM and use this app.
